...
const columns = [
  ...
  { width: 50, label: 'Price', dataKey: 'sell_price', numeric: true },
  { width: 120, label: 'Store', dataKey: 'store' },
  { width: 40, label: 'In Stock', dataKey: 'in_stock' },
  // { width: 40, label: 'Rating', dataKey: 'rating' },
...
const ProductsTable = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (value) {
      const filteredData = rows.filter(row => (
        ...
        row.sell_price.toString().toLowerCase().includes(value.toLowerCase()) ||
        // row.rating.toString().toLowerCase().includes(value.toLowerCase()) ||
  ...
  useEffect(() => {
    fetchData(() => getApi('/product'), setRows, "Unable to fetch products", "Fetching products");
  }, []);
  ...
  return (
    <Container>
      ...
      <TableSearchbar label={"Search Products"} searchTerm={searchTerm} handleSearch={handleSearch} />
      <AddNewTitle title={"Add New Product"} path={`add`} />
      <Paper>
      ...
      <ConfirmationModal
        ...
        handleOnConfirm={() => statusUpdate({ title: "product", dataToSend, successHandler, setLoading })}
        heading={"Update Product status"}
