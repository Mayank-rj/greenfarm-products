...
const columns = [
  { width: 40, label: 'Cover', dataKey: 'cover' },
  { width: 80, label: 'Name', dataKey: 'name' },
  { width: 70, label: 'Phone no.', dataKey: 'mobile', numeric: true },
  { width: 80, label: 'Email', dataKey: 'email' },
  ...
  const handleSearch = (event) => {
    ...
    if (value) {
      const filteredData = rows.filter(row => (
        ...
        row.mobile.toString().toLowerCase().includes(value.toLowerCase()) ||
        row.email.toString().toLowerCase().includes(value.toLowerCase()) ||
        (row.first_name+" " +row.last_name).toLowerCase().includes(searchTerm.trim().toLowerCase())
      )
      );
        ...
        case 'cover':
          return (
            <TableCell key={column.dataKey} align="center">
              <img
                src={row[column.dataKey] ? `${CONFIG.imageURL}${row[column.dataKey]}` : user}
                style={{ width: '50px', height: '50px', objectFit: 'cover' }}
                alt={row["first_name"] || 'No Image'}
              />
            </TableCell>
          );
        case 'name':
...
const handleOpenModal = (_id, value, type) => {
  ...
  const successHandler = () => {
    // fetchData(() => getApi('/category'), setRows, "Unable to fetch categories", "Fetching categories");
    setRows(prev=>{
  ...
  useEffect(() => {
    fetchData(() => getApi('/user/adminuser'), setRows, "Unable to fetch admins", "Fetching admins");
  }, []);
  ...
  return (
    <Container>
      ...
      <TableSearchbar label={"Search Admins"} searchTerm={searchTerm} handleSearch={handleSearch} />
      <AddNewTitle title={"Add New Admin"} path={`add`} />
      <Paper>
      ...
      <ConfirmationModal
        ...
        handleOnConfirm={() => statusUpdate({ title: "user", dataToSend, successHandler, setLoading })}
        heading={"Update Admin status"}
