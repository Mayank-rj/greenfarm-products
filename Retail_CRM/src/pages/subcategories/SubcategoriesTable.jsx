...
const SubcategoriesTable = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (value) {
      const filteredData = rows.filter(row => (
        ...
        row.category?.name.toLowerCase().includes(value.toLowerCase()) ||
  ...
  useEffect(() => {
    fetchData(() => getApi('/subcategory'), setRows, "Unable to fetch subcategories", "Fetching subcategories");
  }, []);
  ...
  return (
    <Container>
      ...
      <TableSearchbar label={"Search Subcategories"} searchTerm={searchTerm} handleSearch={handleSearch} />
      <AddNewTitle title={"Add New Subcategory"} path={`add`} />
      <Paper>
      ...
      <ConfirmationModal
        ...
        handleOnConfirm={() => statusUpdate({ title: "subcategory", dataToSend, successHandler, setLoading })}
        heading={"Update Subcategory status"}
