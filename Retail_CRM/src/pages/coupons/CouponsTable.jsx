...
const columns = [
  { width: 100, label: 'Coupon Code', dataKey: 'coupon_code' },
  { width: 80, label: 'Discount Value', dataKey: 'discount_value' },
  { width: 80, label: 'Discount Type', dataKey: 'discount_type' },
...
const CouponsTable = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (value) {
      const filteredData = rows.filter(row => (
        row.coupon_code.toLowerCase().includes(value.toLowerCase()) ||
        row.discount_type.toLowerCase().includes(value.toLowerCase()) ||
        row.discount_value.toString().toLowerCase().includes(value.toLowerCase())
      )
      );
  ...
  const successHandler = () => {
    // fetchData(() => getApi('/coupon'), setRows, "Unable to fetch coupons", "Fetching coupons");
    setRows(prev => {
  ...
  return (
    <Container>
      ...
      <TableSearchbar label={"Search Coupons"} searchTerm={searchTerm} handleSearch={handleSearch} />
      <AddNewTitle title={"Add New Coupon"} path={`add`} />
      <Paper>
      ...
      <ConfirmationModal
        ...
        handleOnConfirm={() => statusUpdate({ title: "coupon", dataToSend, successHandler, setLoading })}
