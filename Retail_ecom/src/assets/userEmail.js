  ...
  return {
    ...
    html: `
       ...
       <p>Dear Customer (${userprofile.first_name} ${userprofile.last_name}),</p>
        ...
        <table style="border-collapse: collapse; width: 100%; margin-bottom: 20px; max-width: 100%;" class="responsive-table">
        <tbody>
      ...
      JSON.parse(orderDetail?.product_details)
        ?.map(
          (item) => `
              <tr>
                <td style="border: 1px solid #ddd; padding: 8px;">${item.product.name
            }</td>
                <td style="border: 1px solid #ddd; padding: 8px;">
                  <img src="${SITE_CONFIG.imageUrl}/${item.product.cover}" alt="${item.product.name
        ...
        <table style="width: 100%; margin-top: 20px;">
          <tr>
            <td style="width: 150px;">
              <img src=${SITE_CONFIG.imageUrl}/${store?.cover} alt=${store?.name} style="width: 100px; height: auto;" />
            </td>
            <td>
